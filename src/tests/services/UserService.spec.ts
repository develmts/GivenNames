...
import { setupTestDb } from "@/tests/helpers/setupTestDb.js";


import { vi } from "vitest"
import { UsersORM } from "@/orm/UsersORM.js";

// --- MOCK password utils ---
import * as pwUtils from "@/services/ApiService/utils/pwdUtils.js"
vi.spyOn(pwUtils, "hashPassword").mockImplementation(async (pw) => `$mocked-${pw}`)
vi.spyOn(pwUtils, "verifyPassword").mockImplementation(async (pw, hash) => hash === `$mocked-${pw}`)

// killswitch global per saltar proves de DB
const SKIP_DB_TESTS = false;

let UserService: typeof import("../../services/UserService.js").UserService;
...
beforeAll(() => {
  ...
  return import("@/services/UserService.js").then(mod => {
...
describe("UserService", () => {
  ...
  test("should update user profile", async () => {
    ...
    const spy = vi.spyOn(UsersORM.prototype, "updateUserProfile").mockResolvedValue(undefined);
