...
// mock d'AuthService
vi.mock("../../../../services/ApiService/AuthService", () => ({
  AuthService: {
...
describe("Auth routes", () => {
  test("POST /api/auth/login hauria de retornar access token i cookie", async () => {
    const res = await server.app.request("/api/auth/login", {
      method: "POST",
  ...
  test("POST /api/auth/refresh sense cookie ha de fallar", async () => {
    const res = await server.app.request("/api/auth/refresh", {
      method: "POST"
    })
  ...
  test("POST /api/auth/logout hauria de retornar missatge correcte", async () => {
    const res = await server.app.request("/api/auth/logout", {
      method: "POST"
    })
