...
export class GivenNamesORM {
  ...
  //     VALUES (?, ?, ?)
  //   `);
  //   const result = stmt.run(name, locale, gender);
  ...
  //   );
  //   const row = sel.get(name, locale, gender);
  //   return row?.id ?? 0;
  // }
    ...
    try {
      ...
      if (data.variants?.length) {
        for (const variant of data.variants) {
          const variantId = await this.insertName(variant.name, variant.locale, variant.gender);
      ...
      if (data.translations?.length) {
        for (const translation of data.translations) {
          const transId = await this.insertName(translation.name, translation.locale, translation.gender);
      ...
      throw new Error(`Failed atomic insertion for name=${data.name}: ${error}`);
    }
  }
   ...
   * Update a name by id.
   */
  updateName(nameId: number, fields: { name?: string; locale?: string; gender?: string }): void {
  ...
  /**
   * Recupera un nombre con todas sus relaciones (variantes, traducciones, clusters).
   */
  ...
  addVariant(nameId: number, variantId: number): void {
    if (!this.nameExists(nameId) || !this.nameExists(variantId)) {
     throw new Error(`One or both name IDs (${nameId}, ${variantId}) do not exist.`);
    }
    const stmt = this.db.prepare(`
...
listAllVariants(limit = 100, removeDuplicates : boolean = false): any[] {
   ...
   */
  addTranslation(nameId: number, translatedNameId: number, locale: string): void {
    if (!this.nameExists(nameId) || !this.nameExists(translatedNameId)) {
      throw new Error(`One or both name IDs (${nameId}, ${translatedNameId}) do not exist.`);
    }
    const stmt = this.db.prepare(`
...
// considera els canvis fets , marca el punt 8 com fet i ensenyam la llista d pendets globals
  /**
  ...
  testConnection(): boolean {
    try {
      ...
      //   const orm = new GivenNamesORM(config);
    //   orm.connect();
    //   orm.close();
  }
